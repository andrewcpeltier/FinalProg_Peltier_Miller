...
/**
 *  Function called at the end of the initializatApplication() function
 *  to write the intial program execution information to the output file
 *  as specified in the program instructions.
 */ 
void writeInitialOutput();
...
pthread_mutex_t* robotLock;

pthread_mutex_t paneLock;
...
int main(int argc, char** argv)
{
	...
	//Create the outputFile with file name specified in executable params
	...
	//allocate memory for threads and robotLocks
	...
	//Initialize and lock robotThreads
	for(int i = 0; i < numBoxes; i++)
	{
	...
	pthread_mutex_init(&paneLock, NULL);

	gridLock = (pthread_mutex_t**) malloc(numRows*sizeof(pthread_mutex_t*));
	for (int k=0; k<numRows; k++)
		gridLock[k] = (pthread_mutex_t*) malloc((numCols)*sizeof(pthread_mutex_t));
	...
	for(int i = 0; i < numRows; i++)
	{
		for(int j = 0; j < numCols; j++)
		{
			...
			pthread_mutex_unlock(gridLock[i] + j);
		}
	}
	...
	// Now the the program has been executed, close the output file
	fclose(fp);
...
void* moveBox(void *argument) 
{
		...
		//Lock the current robotThread
		pthread_mutex_lock(robotLock+robotThread->robotNum);
		...
		while(!robotThread->reached)
		{
			...
			if(robotThread->boxCol != robotThread->doorCol)
			{
				...
				if(robotThread->doorCol - robotThread->boxCol < 0)
				{
					...
					if(robotThread->robotCol != robotThread->boxCol + 1)
					{
						...
						if(robotThread->robotCol > robotThread->boxCol + 1)
						{
							//unlock the grid coordinate the robot currently resides
							pthread_mutex_unlock(gridLock[robotThread->robotRow]+ robotThread->robotCol);
							//write the robot's movement to the output file
							printMovement(robotThread->robotNum, EAST, 0);
							//Make the movement
							robotThread->robotCol--;
							//Lock the grid coordinate the robot has just moved to
							pthread_mutex_lock(gridLock[robotThread->robotRow]+ robotThread->robotCol);
						}
					...
					else
					{
						...
						{							
							//unlock the grid coordinate the robot currently resides
							pthread_mutex_unlock(gridLock[robotThread->robotRow]+ robotThread->robotCol);// Robot moves North
							//write the robot's movement to the output file
						...
						else
						{
							...
							//unlock the grid coordinate the robot currently resides
							pthread_mutex_unlock(gridLock[robotThread->robotRow]+ robotThread->robotCol);// Push the box West
							pthread_mutex_unlock(gridLock[robotThread->boxRow] + robotThread->boxCol);
							//write the robot's movement to the output file
							...
							//Lock the grid coordinate the robot has just moved to
							pthread_mutex_lock(gridLock[robotThread->robotRow] + robotThread->robotCol);
							pthread_mutex_lock(gridLock[robotThread->boxRow] + robotThread->boxCol);
						}
					}
				}
				...
				else
				{
					...
					else
					{
						...
						if(robotThread->robotRow < robotThread->boxRow)
						{
							...
							//unlock the grid coordinate the robot currently resides
							pthread_mutex_unlock(gridLock[robotThread->robotRow]+ robotThread->robotCol);// Move robot North
							//write the robot's movement to the output file
							printMovement(robotThread->robotNum, NORTH, 0);
							//Make the movement
							robotThread->robotRow++;
						...
						else if (robotThread->robotRow > robotThread->boxRow)
						{
							...
							//unlock the grid coordinate the robot currently resides
							pthread_mutex_unlock(gridLock[robotThread->robotRow]+ robotThread->robotCol);// Move robot South
							//write the robot's movement to the output file
							printMovement(robotThread->robotNum, SOUTH, 0);
							//Make the movement
							robotThread->robotRow--;
						...
						else
						{
							...
							//unlock the grid coordinate the robot currently resides
							pthread_mutex_unlock(gridLock[robotThread->robotRow]+ robotThread->robotCol);// Move the box to the East
							pthread_mutex_unlock(gridLock[robotThread->boxRow]+ robotThread->boxCol);
							//write the robot's movement to the output file
				...
				if(robotThread->doorRow - robotThread->boxRow < 0)
				{
					...
					else
					{
						...
						if(robotThread->robotCol < robotThread->boxCol)
						{
							...
							//unlock the grid coordinate the robot currently resides
							pthread_mutex_unlock(gridLock[robotThread->robotRow]+ robotThread->robotCol);// Moving the robot East
							//write the robot's movement to the output file
							printMovement(robotThread->robotNum, EAST, 0);
							//Make the movement
							robotThread->robotCol++;
						...
						else if (robotThread->robotCol > robotThread->boxCol)
						{
							...
							//unlock the grid coordinate the robot currently resides
							pthread_mutex_unlock(gridLock[robotThread->robotRow]+ robotThread->robotCol);// Moving the West
							//write the robot's movement to the output file
							printMovement(robotThread->robotNum, WEST, 0);
							//Make the movement
							robotThread->robotCol--;
						...
						else
						{
							...
							//unlock the grid coordinate the robot currently resides
							pthread_mutex_unlock(gridLock[robotThread->robotRow]+ robotThread->robotCol);// Moving the box North
							pthread_mutex_unlock(gridLock[robotThread->boxRow]+ robotThread->boxCol);
							//write the robot's movement to the output file
				...
				else
				{
					...
					if(robotThread->robotRow != robotThread->boxRow - 1)
					{
						...
						if(robotThread->robotRow > robotThread->boxRow - 1)
						{
							...
							//unlock the grid coordinate the robot currently resides
							pthread_mutex_unlock(gridLock[robotThread->robotRow]+ robotThread->robotCol);// Moving the robot West
							//write the robot's movement to the output file
							printMovement(robotThread->robotNum, WEST, 0);
							//Make the movement
							robotThread->robotRow--;
						...
						else
						{
							...
							//unlock the grid coordinate the robot currently resides
							pthread_mutex_unlock(gridLock[robotThread->robotRow]+ robotThread->robotCol);// Moving robot East
							//write the robot's movement to the output file
							printMovement(robotThread->robotNum, EAST, 0);
							//Make the movement
							robotThread->robotRow++;
					...
					else
					{
						...
						else
						{
							...
							//unlock the grid coordinate the robot currently resides
							pthread_mutex_unlock(gridLock[robotThread->robotRow]+ robotThread->robotCol);// Moving the box South
							pthread_mutex_unlock(gridLock[robotThread->boxRow]+ robotThread->boxCol);
							//write the robot's movement to the output file
			...
			//Sleep the robot thread the specified time
...
void initializeApplication(void)
{
	...
	//Write the initial program execution information to the output text file
	writeInitialOutput();
